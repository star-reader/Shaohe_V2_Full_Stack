    ...
    <section class="wrapper">
      ...
      <section class="main-content">
        <form @submit.prevent="register">
          ...
          <div class="line"></div>
          <input type="password" placeholder="确认密码" v-model="conformPassword">
          <div class="line"></div>
          <input type="text" placeholder="邀请码（选填）" v-model="invatationCode">
          <button>注册</button>
        </form>
      </section>
      <footer>
        <!-- <p><a href="" title="Forgot Password">忘记密码</a></p> -->
        <p @click="gotoLogin"><a title="Register">已有账号，直接登录</a></p>
      </footer>
    </section>
  </section>
  </div>
...
const register = async () => {
  ...
  try {
    const response = await axios.post(api.register, {
      ...
      invatationCode: invatationCode.value
    })
